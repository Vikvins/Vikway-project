...
MODE_WEIGHT_CANDIDATES: dict[Mode, list[str]] = {
    ...
    "quiet": ["w_quiet", "w_noise", "weight"],
    "green": ["w_green", "weight"],
    "balanced": ["w_balanced", "w_accessible", "weight", "w_short"],
}
...
def _load_spatial_index(export_dir: Path, file_name: str) -> STRtree | None:
...
def _estimate_noise_db(
    ...
    if rail_index is not None:
        try:
            near = rail_index.query(edge_line.buffer(60.0))
            if len(near) > 0:
                rail_component = 6.0
        except Exception:
            rail_component = 0.0
...
def _estimate_green_score(
    ...
    if green_index is not None:
        try:
            close_10 = len(green_index.query(edge_line.buffer(10.0)))
            close_25 = len(green_index.query(edge_line.buffer(25.0)))
            close_50 = len(green_index.query(edge_line.buffer(50.0)))
